...
module.exports = (app, db) => {
	...
	app.post('/api/v1/user/changePassword/:key_id', async (req, res) => {
		...
		if (req.body.password1 !== req.body.password2) {
			res.json({ status: 400, msg: "Vos deux mots de passe ne sont pas identique !" });
		} else {
			let result = await userModel.updatePassword(req.body.password1, key_id);
			if (result.code) {
				res.json({ status: 400, msg: "le mot de passe ne s'est pas modifié !" });
			} else {
				res.json({ status: 200, msg: "le mot de passe a bien été modifié !" });
			}
		}
	})
